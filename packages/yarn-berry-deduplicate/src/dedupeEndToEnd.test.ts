...
const dedupeBin = ((): string => {
  ...
  if (!built) {
    throw new Error(
      "yarn-berry-deduplicate is not built: run `bun run build` first",
    );
  }
  return built;
})();
...
const install = (cwd: string, ...args: string[]): Run =>
  run(cwd, "yarn", ["install", ...args]);

const dedupe = (cwd: string, ...args: string[]): Run =>
  run(cwd, process.execPath, [dedupeBin, ...args]);

const resolutionsOf = (dir: string, packageName: string): string[] => {
  const packagesMap = buildYarnPackagesMap(
    parseYarnLockPackages(readAndParseYarnLock(join(dir, "yarn.lock"))),
  );
...
const setNodeLinker = (dir: string, nodeLinker: NodeLinker): void => {
  const path = join(dir, ".yarnrc.yml");
  const yarnrc = readFileSync(path, "utf8");
  const updated = yarnrc.replace(
    /^nodeLinker:.*$/mu,
...
const linkedVersions = (
  ...
  const pattern = new RegExp(
    `${packageName}-npm-(?<version>.+?)-[0-9a-f]{10}-`,
    "gu",
  );
  const versions = [
