...
const fixesFor = (fixture: string): Map<string, ResolutionFix[]> => {
...
const mergesFor = (fixture: string, packageName: string): ResolutionFix[] =>
  (fixesFor(fixture).get(packageName) ?? []).filter(
    (fix) => fix.mergeableResolutions.length > 1,
  );
...
describe("identifyResolutionFixes over yarn lockfiles", () => {
  ...
  it("merges an aliased range onto the direct one", () => {
    expect(
      mergesFor("mergeable-alias", "printable-shell-command"),
    ).toHaveLength(1);
  });

  it("leaves an aliased range that covers nothing else alone", () => {
    expect(
      mergesFor("aliased-range-constrains-merge", "printable-shell-command"),
    ).toEqual([]);
  });
  ...
  it("finds nothing to merge in a lockstep family", () => {
    expect(
      mergesFor("duplicated-typescript-eslint", "@typescript-eslint/types"),
    ).toEqual([]);
    expect(
      mergesFor("duplicated-typescript-eslint", "@typescript-eslint/utils"),
    ).toEqual([]);
  });
