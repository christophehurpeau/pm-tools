...
import { addResolutions } from "./helpers/packageJsonResolutions.ts";
...
export interface ClusterApplyOutcome {
  ...
  // resolutions the duplicates came back without: reported, never left behind
...
export interface ApplyClusterFixesOptions {
  ...
  // what `yarn install` itself would still change, for the dry-run report. Only
...
const defaultReadFixes = (projectDir: string): ClusterFix[] => {
  ...
  return identifyClusterFixes(
    buildYarnPackagesMap(packages),
    packages,
    collectWorkspaces(packages, createManifestReader(projectDir)),
  );
};
...
export const applyClusterFixes = ({
  ...
  const skipped = [
    ...
    ...rejected.flatMap(({ override, rejectedBy }) =>
      rejectedBy.map(
        (constraint) =>
          `resolution ${describeOverride(override)}: ${constraint.requesterName ?? "workspace"} requires "${constraint.range}", and a yarn resolution would force it too`,
      ),
    ),
  ];
  ...
  const frozenFailure = (): ClusterApplyOutcome | null => {
    ...
    log(
      "  `yarn install --immutable` rejects the result — reverting, the lockfile it produced is not one CI can reuse",
    );
  ...
  for (const override of outstanding) {
    ...
    if (aliased.length > 0) {
      log(
        `    every requester of ${override.packageName} declares it through an alias (${aliased.join(", ")}); a yarn resolution repoints the target, not those keys`,
      );
    }
  }
  writeFileSync(
  ...
  if (returnedDuplicates.length > 0 || returnedReuses.length > 0) {
    log(
      "  The resolutions were the only thing holding the result: yarn resolves back without them. A fix that needs a standing resolution is not one this tool applies.",
    );
