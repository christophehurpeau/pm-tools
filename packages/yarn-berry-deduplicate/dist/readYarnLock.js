...
import { parseYarnLock } from "./helpers/syml.js";
/**
 ...
 */
export const readAndParseYarnLock = (filepath) => {
    const entries = parseYarnLock(readFileSync(filepath, "utf8"));
    if (!entries.__metadata) {
        throw new Error(`${filepath} carries no __metadata: it is not a yarn berry lockfile. yarn classic's format is not one this tool reads.`);
    }
    return entries;
};
