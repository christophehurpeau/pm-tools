...
`));
        expect(packages.get("lodash@npm:^4.0.0")).toBe(packages.get("lodash@npm:^4.17.0"));
    });
    ...
    it("names an aliased descriptor after the package it resolves to", () => {
        ...
        expect(packages.get("psc@npm:printable-shell-command@^5.0.0")).toMatchObject({
            type: "npm",
            name: "printable-shell-command",
            version: "5.0.7",
        });
    ...
    it("keeps non-npm protocols out of the npm pool", () => {
        ...
        expect(packages.get("local-lib@workspace:packages/local-lib")).toMatchObject({ type: "other", protocol: "workspace" });
        expect(packages.get("resolve@patch:resolve@npm%3A^1.22.8#optional!builtin<compat/resolve>")).toMatchObject({ type: "other", protocol: "patch" });
...
`));
        const resolutions = new Set([...packages.values()].map((pkg) => pkg.resolution));
        expect([...resolutions]).toEqual(["react-dom@npm:18.3.1"]);
    });
});
//# sourceMappingURL=parseYarnLockPackages.test.js.map