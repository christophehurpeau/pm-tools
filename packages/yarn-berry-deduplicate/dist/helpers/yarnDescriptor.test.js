...
describe("parseYarnDescriptor", () => {
    ...
    it("takes an alias's requested range from its target", () => {
        expect(parseYarnDescriptor("psc@npm:printable-shell-command@^5.0.0")).toMatchObject({
            key: "psc",
    ...
    it("reads the non-npm protocols yarn writes", () => {
        ...
        expect(parseYarnDescriptor("resolve@patch:resolve@npm%3A^1.22.8#optional!builtin<compat/resolve>")).toMatchObject({ npmName: "resolve", protocol: "patch" });
    });
    it("refuses a descriptor carrying no range", () => {
        expect(() => parseYarnDescriptor("lodash")).toThrow("Invalid yarn descriptor without range: lodash");
    });
});
describe("splitEntryKey", () => {
    it("splits the descriptors a lockfile key covers", () => {
        expect(splitEntryKey("@babel/code-frame@npm:7.10.4, @babel/code-frame@npm:~7.10.4")).toEqual([
...
//# sourceMappingURL=yarnDescriptor.test.js.map