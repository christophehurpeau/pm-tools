...
import { splitEntryKey } from "./yarnDescriptor.js";
const resolutionOf = (entryKey, entry) => entry.resolution ?? splitEntryKey(entryKey)[0];
/**
 ...
 * alone.
 */
export const applyIdentifiedFixesToYarnLock = (entries, identifiedFixesMap) => {
    ...
    for (const fixes of identifiedFixesMap.values()) {
        for (const fix of fixes) {
            for (const resolution of fix.mergeableResolutions) {
                if (resolution !== fix.to)
                    declaredTargets.set(resolution, fix.to);
            }
        }
    }
    /**
     ...
     */
    const finalTargetOf = (resolution) => {
    ...
    const targetByResolution = new Map([...declaredTargets.keys()]
        .map((resolution) => [resolution, finalTargetOf(resolution)])
    ...
    const entryByResolution = new Map();
    ...
    for (const [entryKey, entry] of packageEntries(entries)) {
        ...
        const [resolution, groupEntry] = (() => {
            if (target === undefined)
                return [ownResolution, entry];
    ...
    if (entries.__metadata)
        newEntries.__metadata = entries.__metadata;
...
//# sourceMappingURL=applyIdentifiedFixesToYarnLock.js.map