...
const dependentsOf = (fixture, packageName) => {
    const { packages, workspaces } = loadFixture(fixture);
    return (collectYarnDependents({
        packages,
        workspaces,
...
describe("collectYarnDependents", () => {
    it("records the range each requester declares and what it got", () => {
        const dependents = dependentsOf("duplicated-printable-shell-command", "printable-shell-command");
        expect(dependents.map(({ key, version, workspace, resolvedVersion }) => ({
            key,
            version,
            workspace,
            resolvedVersion,
    ...
    it("names the declaration an aliased range comes from", () => {
        const dependents = dependentsOf("mergeable-alias", "printable-shell-command");
        expect(dependents.map(({ aliasKey, version }) => ({ aliasKey, version }))).toEqual([
    ...
    it("tells a workspace's dependency block from its devDependency block", () => {
        expect(dependentsOf("workspaces", "semver").map(({ key, workspace }) => ({
            key,
            workspace,
        }))).toEqual([
            {
    ...
    it("flags declarations semver cannot read, keeping them as written", () => {
        ...
        expect(dependents.get("from-git")?.map(({ version, nonSemver }) => ({
            version,
            nonSemver,
        }))).toEqual([
            {
        ...
        expect(dependents.get("local-lib")?.map(({ version, nonSemver }) => ({
            version,
            nonSemver,
    ...
    it("files a patch declaration under the resolution it names", () => {
        ...
        expect(dependents.map(({ key, version, resolvedVersion, resolvedResolution }) => ({
            key,
            version,
            resolvedVersion,
            resolvedResolution,
            ...
            },
            {
                ...
                resolvedResolution: "lodash@patch:lodash@npm%3A4.17.21#./patches/lodash.patch::version=4.17.21&hash=1a2b3c",
            },
        ]);
    });
    ...
    it("counts a constraint once when a patch entry repeats it", () => {
        const { packages, workspaces } = loadFixture("non-npm");
        const resolveDependents = collectYarnDependents({ packages, workspaces }).get("resolve") ?? [];
    ...
    it("counts a peerDependencies range as a constraint", () => {
        ...
        expect(dependents.map(({ key, version, peer, resolvedVersion }) => ({
            key,
            version,
            peer,
            resolvedVersion,
...
//# sourceMappingURL=collectYarnDependents.test.js.map