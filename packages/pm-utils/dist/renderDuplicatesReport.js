...
const constraintText = (requesterName) => requesterName ?? "workspace";
const renderPackage = (view, clusterIndex, color, log) => {
    log();
    ...
    if (view.dependents.length > 0) {
        ...
        for (const dependent of view.dependents) {
            const resolved = dependent.resolvedVersion !== undefined &&
                dependent.resolvedVersion !== dependent.range
                ? `, resolved ${color("yellow", dependent.resolvedVersion)}`
                : "";
            log(`    - ${dependent.requester} requires ${color("yellow", `"${dependent.range}"`)}${resolved}`);
        }
    }
    if (view.dedupe.length > 0) {
        log(`  ${color("dim", "Dedupe:")}`);
        for (const { from, to, direction } of view.dedupe) {
            log(`    - ${color("yellow", from.join(", "))} -> ${color("green", to)}${directionLabel(direction ?? "none")}`);
        }
    }
};
const renderCluster = (fix, index, color, log) => {
    ...
    log();
    log(`cluster ${index} — ${color("bold", clusterLabel(fix.members))} [${counts}]:`);
    ...
    if (fix.applicable && fix.target !== null) {
        ...
        if (fix.driverMembers.length > 0) {
            ...
            const follow = followers > 0
                ...
                : "";
            log(`    ${color("dim", "Driven by:")} ${fix.driverMembers.join(", ")}${follow}`);
        }
        ...
        if (fix.floatingMembers.length > 0) {
            log(`    ${color("dim", "Resolver picks:")} ${fix.floatingMembers.join(", ")}`);
        }
    }
    if (fix.excludedMembers.length > 0) {
        ...
        for (const excluded of fix.excludedMembers) {
            const blockers = excluded.blockedBy
                .map((constraint) => `${constraintText(constraint.requesterName)} requires ${color("yellow", `"${constraint.range}"`)}`)
                .join(", ");
    ...
    if (fix.reuseFixes.length > 0) {
        ...
        for (const reuse of fix.reuseFixes) {
            log(`    - ${reuse.requesterName} requires ${reuse.packageName} ${color("yellow", `"${reuse.range}"`)}, resolved ${color("yellow", reuse.from)} -> would pin ${color("green", reuse.to)}`);
        }
    }
    if (fix.externalConstraints.length > 0) {
        log(`  ${color("dim", "External constraints:")}`);
        for (const constraint of fix.externalConstraints) {
            log(`    - ${constraintText(constraint.requesterName)} requires ${constraint.packageName} ${color("yellow", `"${constraint.range}"`)}`);
        }
    }
};
// A package counts once, whether its own resolutions can merge, its cluster
// converges it, or both.
const countDedupable = (packages, clusters) => {
...
export const renderDuplicatesReport = ({ title, packages, totalDependencies, clusterFixes = [], dedupeCommand, color: colorEnabled = shouldColorize(), log = console.log, }) => {
