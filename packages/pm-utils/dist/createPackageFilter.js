...
const scopePattern = (scope) => `${scope.startsWith("@") ? scope : `@${scope}`}/**`;
const toPatterns = (names, scopes) => [
    ...names,
...
const compile = (patterns) => patterns.length === 0 ? () => false : picomatch(patterns);
/**
 ...
 * exact match.
 */
export const createPackageFilter = ({ include = [], exclude = [], includeScopes = [], excludeScopes = [], } = {}) => {
    ...
    const rejectionReason = (packageName) => {
    ...
    return {
        selectsEverything: includePatterns.length === 0 && excludePatterns.length === 0,
...
export const selectPackages = (packages, filter) => filter.selectsEverything
    ? packages
    : Object.fromEntries(Object.entries(packages).filter(([packageName]) => filter.selects(packageName)));
/**
 ...
 */
export const selectClusterFixes = (fixes, filter) => {
    if (filter.selectsEverything)
        return { selected: fixes, skipped: [] };
    const selected = [];
    const skipped = [];
...
export const describeSkippedClusterFix = ({ fix, blockedBy, }) => {
...
//# sourceMappingURL=createPackageFilter.js.map