...
const renderSummary = (overrides = {}) => render({ details: false, ...overrides });
...
describe("renderDuplicatesReport", () => {
    ...
    it("files a dependent under the versionless resolution it names", () => {
        ...
        ok(output.indexOf("  metro@patch:metro@npm%3A0.87.0#./p.patch") <
    ...
    it("attributes an unresolved dependent to the sole version", () => {
        ...
        ok(output.includes('    - mini-deep@2.0.0                  requires "^0.84.0" (peer)'));
        ok(output.includes('    - old-deep@1.0.0                   requires "^0.74.0" (peer)'));
    });
    it("lists the installation contexts of a shared resolution", () => {
        ...
        ok(output.includes("    installed at: metro@0.84.5, metro@0.84.5(react@19)"));
    });
    it("states how a duplicate would collapse", () => {
        ...
        ok(output.includes("metro — 2 versions, can be deduped to 0.87.0 (upgrade)"));
    ...
    it("leads with the notice instead of a count that would read as no report", () => {
        ...
        ok(output.startsWith("lodash is not duplicated. Showing its dependents:\n"));
        ok(!output.includes("No duplicates found"));
        ok(output.includes('    - eslint@9.0.0  requires "^4.17.0"'));
        strictEqual(lastLine(output), "Found 120 dependencies, 1 match, 0 dedupable.");
    });
    ...
    it("credits a cluster for a package its own resolutions cannot fix", () => {
        ...
        ok(output.includes("metro — 2 versions, can be deduped to 0.87.0 (upgrade)"));
        strictEqual(lastLine(output), "Found 120 dependencies, 1 duplicate, at least 1 dedupable (deduping may remove more). Run `pnpm-dedupe` to apply.");
    });
    ...
    it("words the dedupable count as a floor", () => {
        ok(render({ clusterFixes: [clusterFix()] }).includes("at least 1 dedupable (deduping may remove more)"));
    });
    ...
    it("files an external range under the version it resolved to", () => {
        ...
        ok(output.includes("    (resolved version unknown)"));
        ok(output.includes('      - react-native-web  requires metro-config "^0.74.1"'));
    });
    ...
    it("gives each package one line, with its census and its verdict", () => {
        ...
        ok(output.includes("- metro  resolved to 2 versions (0.87.0, 0.84.5), can be deduped to 0.87.0 (upgrade)"));
    });
    ...
    it("points at the tree", () => {
        ok(renderSummary().includes("Run `pnpm-why-duplicate --details` to see every dependent."));
    ...
    it("names the install paths of a package resolved once", () => {
        ...
        ok(output.includes("- metro  resolved to 1 version (0.84.5) installed at 2 paths"));
    });
    it("styles without moving anything: stripped, the two renders match", () => {
        ...
        const stripped = render({ ...options, color: true }).replaceAll(escapePattern, "");
