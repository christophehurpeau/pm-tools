 ...
 * other fading out.
 */
export const stylePackageName = (color, name, styles = ["packageName"]) => {
    // 0 when unscoped, since indexOf returns -1
    const scopeEnd = name.startsWith("@") ? name.indexOf("/") + 1 : 0;
    if (scopeEnd === 0)
        return color(styles, name);
    const scopeStyles = styles.map((style) => style === "packageName" ? "scope" : style);
...
const versionStart = (reference) => {
    const nameStart = reference.startsWith("@") ? reference.indexOf("/") + 1 : 0;
    if (nameStart === 0 && reference.startsWith("@"))
        return -1;
