...
import { partitionUnconditionalOverrides } from "./unconditionalOverrides.js";
const constraint = (range) => ({
    requester: "requester",
    requesterName: "requester",
    packageName: "lib",
    range,
});
const workspaceConstraint = (range) => ({
    ...constraint(range),
    requester: "package.json in dependencies",
    requesterName: undefined,
});
// only `externalConstraints` is read; the rest is an empty cluster
const fixWith = (constraints) => ({
    members: ["lib"],
...
describe("partitionUnconditionalOverrides", () => {
    it("rejects an override a third-party range does not accept", () => {
        const { safe, rejected } = partitionUnconditionalOverrides([fixWith([constraint("^1.0.0")])], [override]);
        expect(safe).toEqual([]);
    ...
    it("keeps an override every range accepts", () => {
        const { safe, rejected } = partitionUnconditionalOverrides([fixWith([constraint("^2.0.0")])], [override]);
    ...
    it("ignores a workspace constraint", () => {
        const { safe } = partitionUnconditionalOverrides([fixWith([workspaceConstraint("^1.0.0")])], [override]);
        expect(safe).toEqual([override]);
    });
});
//# sourceMappingURL=unconditionalOverrides.test.js.map