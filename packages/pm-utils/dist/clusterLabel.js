...
const covers = (prefix, member) => member === prefix ||
...
const formatGroup = ({ prefix, covered }) => {
    if (covered.length === 1 && covered[0] === prefix)
        return prefix;
 ...
 */
export const clusterLabel = (members) => {
    ...
    if (second !== null && second.covered.length > 1)
        groups.push(second);
    const uncovered = members.filter((member) => !groups.some((group) => covers(group.prefix, member))).length;
    const label = groups.map(formatGroup).join(" + ");
    return uncovered === 0 ? label : `${label} (+${uncovered} more)`;
};
//# sourceMappingURL=clusterLabel.js.map