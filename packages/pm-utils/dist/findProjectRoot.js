 ...
 * workspace above it.
 */
export const findProjectRoot = ({ lockfileName, cwd = process.cwd(), }) => {
    let dir = cwd;
    for (;;) {
        if (existsSync(join(dir, lockfileName)))
            return dir;
 ...
 * parseable.
 */
export const resolveProjectDir = ({ lockfileName, cwd = process.cwd(), }) => {
