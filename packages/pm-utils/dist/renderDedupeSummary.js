import { stylePackageName } from "./packageStyles.js";
...
const versionList = (color, versions, style) => `${plural(versions.length, "version", "versions")} (${listText({
    items: versions,
 ...
 */
export const renderDedupeSummary = ({ deduped, remainingDuplicates, whyCommand, color: colorEnabled = shouldColorize(), log = console.log, }) => {
    if (deduped.length === 0)
        return;
    const color = createColorize(colorEnabled);
    const mergedCount = deduped.reduce((total, entry) => total + (entry.before.length - entry.after.length), 0);
    log(`${color("bold", "Deduped")} ${plural(deduped.length, "package", "packages")}, ${plural(mergedCount, "copy", "copies")} merged:`);
    // tighter than the shared budget: two version lists follow the name here
    const width = columnWidth(deduped.map((entry) => entry.packageName), 34);
    for (const { packageName, before, after } of deduped) {
        log(`  ${stylePackageName(color, packageName)}:${padTo(width, packageName)} ${versionList(color, before, "yellow")} -> ${versionList(color, after, "green")}`);
    }
    log(remainingDuplicates === 0
        ? "No duplicate left."
        : `${plural(remainingDuplicates, "duplicate", "duplicates")} left — run ${color("cyan", `\`${whyCommand}\``)} to see ${remainingDuplicates === 1 ? "it" : "them"}.`);
};
//# sourceMappingURL=renderDedupeSummary.js.map