...
const fixFor = (dependentsMap: ClusterDependentsMap, map = members) =>
  identifyLockstepClusterFixes([["leaf", "root"]], map, dependentsMap)[0]!;
...
describe("identifyLockstepClusterFixes", () => {
  ...
  it("proposes the pin change only when nothing else deduplicates", () => {
    ...
    expect(fix.convergentMembers).toEqual(["leaf"]);
    expect(
      fix.workspaceChanges.map((change) => `${change.range} -> ${change.to}`),
    ).toEqual(["~1.0.0 -> 2.0.0"]);
  });
