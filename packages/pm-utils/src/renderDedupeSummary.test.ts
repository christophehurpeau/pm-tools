...
import { renderDedupeSummary } from "./renderDedupeSummary.ts";
import type { DedupeSummaryOptions } from "./renderDedupeSummary.ts";

const render = (overrides: Partial<DedupeSummaryOptions> = {}): string => {
  let output = "";
  renderDedupeSummary({
    deduped: [
...
const lines = (output: string): string[] => output.trimEnd().split("\n");
...
describe("renderDedupeSummary", () => {
  ...
  it("keeps the surviving version alongside its count", () => {
    ok(
      render().includes(
        "  semver: 2 versions (7.5.4, 7.7.1) -> 1 version (7.7.1)\n",
      ),
    );
  });
  ...
  it("names what is left of a package only partly collapsed", () => {
    ok(
      render({
        ...
        "  react: 3 versions (17.0.2, 18.2.0, 19.0.0) -> 2 versions (18.2.0, 19.0.0)\n",
      ),
    );
  });
  ...
  it("summarises a package resolved more times than the line can hold", () => {
    ok(
      render({
        ...
        "6 versions (18.0.0, 20.0.0, 20.1.0, 22.0.0, 22.1.0, +1 more) -> 1 version (24.0.0)",
      ),
    );
  });
  ...
  it("points at the why command for what is left", () => {
    strictEqual(
      lines(render({ remainingDuplicates: 3 })).at(-1),
      "3 duplicates left — run `bun-why-duplicate` to see them.",
    );
    strictEqual(
      lines(render({ remainingDuplicates: 1 })).at(-1),
      "1 duplicate left — run `bun-why-duplicate` to see it.",
    );
  });
