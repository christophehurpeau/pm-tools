...
const clusterFix = (members: string[]): ClusterFix => ({
  members,
...
describe("selectClusterFixes", () => {
  ...
  it("keeps a fix whose whole family is selected", () => {
    ...
    const { selected, skipped } = selectClusterFixes(
      [fix],
      createPackageFilter({ includeScopes: ["@babel"] }),
    );
  ...
  it("skips a partly selected family and names what blocked it", () => {
    ...
    const { selected, skipped } = selectClusterFixes(
      [fix],
      createPackageFilter({ include: ["@babel/types"] }),
    );
