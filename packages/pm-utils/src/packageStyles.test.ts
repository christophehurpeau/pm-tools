...
import { createColorize } from "./reportColors.ts";
...
const plainText = (text: string): string => text.replaceAll(escapePattern, "");

describe("stylePackageName", () => {
  it("returns the name untouched when colour is off", () => {
    strictEqual(
      stylePackageName(createColorize(false), "@babel/core"),
      "@babel/core",
    );
  });
  ...
  it("switches to 24-bit colour when the terminal advertises it", () => {
    ...
    ok(
      stylePackageName(colorize, "metro").startsWith("\u001B[38;2;215;135;95m"),
    );
  });
...
describe("stylePackageReference", () => {
  ...
  it("leaves a reference carrying no version untouched", () => {
    strictEqual(
      stylePackageReference(colorize, "package.json in devDependencies"),
