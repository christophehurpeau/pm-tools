...
describe("clusterLabel", () => {
  it("names a family by its shared scope", () => {
    strictEqual(
      clusterLabel(typescriptEslint),
      "@typescript-eslint/* (+1 more)",
    );
  });

  it("names the two largest groups and counts the rest", () => {
    strictEqual(
      clusterLabel(metroFamily),
      "@react-native/* + metro* (+2 more)",
    );
  });
  ...
  it("keeps the member name when it is the prefix", () => {
    strictEqual(
      clusterLabel(["mini-metro", "mini-metro-config"]),
      "mini-metro*",
    );
  });
  ...
  it("does not name a second group that covers one member", () => {
    strictEqual(
      clusterLabel(["metro", "metro-config", "ob1"]),
      "metro* (+1 more)",
    );
  });
