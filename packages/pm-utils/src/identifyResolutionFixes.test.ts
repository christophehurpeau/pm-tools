...
const npm = (
  name: string,
  version: string,
): ResolutionEntry<NpmResolutionPackage> => ({
  resolution: `${name}@${version}`,
  package: { type: "npm", name, version },
});

const workspace = (name: string): ResolutionEntry => ({
...
const dependentsOf = (
  name: string,
  ranges: string[],
): Map<string, ResolutionDependent[]> =>
  new Map([[name, ranges.map((version) => ({ version }))]]);
...
describe("identifyResolutionFixes", () => {
  it("ignores a dependent whose declaration semver cannot read", () => {
    ...
    const dependents = new Map<string, ResolutionDependent[]>([
      [
        "lib",
  ...
  it("merges every resolution onto the highest version satisfying all dependents", () => {
    const fixes = identifyResolutionFixes(
      [npm("lib", "1.0.0"), npm("lib", "1.2.0")],
      dependentsOf("lib", ["^1.0.0", "^1.2.0"]),
    );
  ...
  it("partitions greedily when no version satisfies every dependent", () => {
    const fixes = identifyResolutionFixes(
      [npm("lib", "1.0.0"), npm("lib", "1.2.0"), npm("lib", "2.0.0")],
      dependentsOf("lib", ["^1.0.0", "^1.2.0", "^2.0.0"]),
    );
  ...
  it("refuses a merge that would drop a dependent the merged resolution served", () => {
    ...
    const fixes = identifyResolutionFixes(
      [npm("lib", "1.0.0"), npm("lib", "1.2.0"), npm("lib", "2.0.0")],
      dependentsOf("lib", [">=1.0.0", "^1.0.0", "2.0.0"]),
    );
  ...
  it("ignores resolutions that are not npm packages", () => {
    const fixes = identifyResolutionFixes(
      [npm("lib", "1.0.0"), workspace("lib"), npm("lib", "1.2.0")],
      dependentsOf("lib", ["^1.0.0", "^1.2.0"]),
    );
