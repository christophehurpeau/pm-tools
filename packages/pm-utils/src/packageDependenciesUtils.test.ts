...
type ParseCase = readonly [
  declaration: readonly [key: string, value: string],
  expected: PackageDependencyDescriptor,
];

const npmCases: ParseCase[] = [
  [
    ["name", "1.1.1"],
    {
      key: "name",
...
const otherProtocolCases: ParseCase[] = [
  [
    ["name", "workspace:*"],
    {
      key: "name",
...
const shorthandCases: ParseCase[] = [
  [
    ["name", "user/repo"],
    {
      key: "name",
...
const expectParsed = (cases: ParseCase[]): void => {
  for (const [[key, value], expected] of cases) {
    expect(PackageDependencyDescriptorUtils.parse(key, value)).toEqual(
      expected,
    );
  }
};
...
describe("PackageDependencyDescriptorUtils.parse", () => {
  ...
  it("rejects a scoped name without a segment", () => {
    expect(() =>
      PackageDependencyDescriptorUtils.parse("name", "npm:@scope"),
    ).toThrow("Invalid package name with scope");
  });

  it("does not read an alias out of a non-npm protocol", () => {
    expect(
      PackageDependencyDescriptorUtils.parse("name", "workspace:@scope/x@1"),
    ).toEqual({
      key: "name",
      npmName: "name",
      nameDescriptor: { name: "name" },
...
describe("PackageDependencyDescriptorUtils.stringify", () => {
  it("round trips every declaration byte for byte", () => {
    for (const [[key, value]] of allCases) {
      expect(
        PackageDependencyDescriptorUtils.stringify(
          PackageDependencyDescriptorUtils.parse(key, value),
        ),
      ).toEqual([key, value]);
    }
  });
...
describe("isSemverComparable", () => {
  const comparableOf = (key: string, value: string): boolean =>
    isSemverComparable(PackageDependencyDescriptorUtils.parse(key, value));
