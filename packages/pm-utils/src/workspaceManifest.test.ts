...
import { applyWorkspaceRangeEdit, nextSelector } from "./workspaceManifest.ts";
...
describe("applyWorkspaceRangeEdit", () => {
  it("edits only the declaration in the requested depType", () => {
    strictEqual(
      applyWorkspaceRangeEdit(manifest, {
        packageName: "metro",
  ...
  it("keeps the npm: alias prefix", () => {
    strictEqual(
      applyWorkspaceRangeEdit(manifest, {
        ...
        range: "^0.84.5",
        to: "0.87.0",
      })?.includes('"aliased": "npm:metro@^0.84.5"'),
      true,
    );
  ...
  it("rewrites past siblings declared with protocols it does not know", () => {
    strictEqual(
      applyWorkspaceRangeEdit(manifestWithProtocols, {
        packageName: "metro",
        depType: "dependencies",
        range: "^0.84.5",
        to: "0.87.0",
  ...
  it("edits an aliased declaration through its own key", () => {
    strictEqual(
      ...
      )?.includes('"aliased": "npm:metro@^0.87.0"'),
      true,
    );
  });
  ...
  it("edits the key an alias reaches the package through, not its namesake", () => {
    ...
    strictEqual(
      applyWorkspaceRangeEdit(swapped, {
    ...
    strictEqual(
      ...
      swapped.replace(
        ...
        '"npm:@typescript/typescript6@6.1.0"',
      ),
    );
  });
