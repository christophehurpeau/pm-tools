...
describe("buildPnpmPackagesMap", () => {
    ...
    it("detects the metro family a non-reused wildcard duplicated", () => {
        ...
        const duplicates = filterDuplicatesPnpmPackagesMap(buildFor("wildcard-not-reused"));
        const family = Object.keys(duplicates).filter((name) => name === "metro" || name.startsWith("metro-"));
        strictEqual(family.length, 14);
        for (const name of family) {
            deepStrictEqual(versionsOf(duplicates, name), new Set(["0.84.5", "0.87.0"]));
        }
    });
    ...
    it("identifies packages by their id when alias keys are swapped", () => {
        const duplicates = filterDuplicatesPnpmPackagesMap(buildFor("aliased-swapped-names"));
        deepStrictEqual(Object.keys(duplicates), ["typescript"]);
        deepStrictEqual(versionsOf(duplicates, "typescript"), new Set(["5.9.3", "7.0.2"]));
    });
});
