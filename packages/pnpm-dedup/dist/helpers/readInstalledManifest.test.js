...
import { createManifestReaderWithStats } from "./readInstalledManifest.js";
const hoistedFixture = fileURLToPath(new URL("../../test/fixtures/hoisted-node-linker", import.meta.url));
describe("createManifestReaderWithStats", () => {
    it("reads a hoisted layout, top-level and nested", () => {
        const { readManifest, stats } = createManifestReaderWithStats(hoistedFixture);
    ...
    it("reads pnpm's virtual store layout, peer suffix included", () => {
        ...
        try {
            const packageDir = join(dir, "node_modules", ".pnpm", "@scope+pkg@1.2.3(react@19.0.0)", "node_modules", "@scope", "pkg");
    ...
    it("honors a relocated virtual store", () => {
        ...
        try {
            const packageDir = join(dir, "node_modules", ".pnpm-store", "pkg@1.0.0", "node_modules", "pkg");
            mkdirSync(packageDir, { recursive: true });
            writeFileSync(join(packageDir, "package.json"), JSON.stringify({ name: "pkg", version: "1.0.0", dependencies: {} }));
    ...
    it("reports every read as missed when nothing is installed", () => {
        const { readManifest, stats } = createManifestReaderWithStats(join(tmpdir(), "pnpm-dedup-not-installed"));
...
//# sourceMappingURL=readInstalledManifest.test.js.map