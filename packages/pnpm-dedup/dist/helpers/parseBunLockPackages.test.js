...
describe.skip("parseBunLockPackages", () => {
    it("parses simple bun.lock fixture", () => {
        ...
        strictEqual(semver.integrity, "sha512-SdsKMrI9TdgjdweUSR9MweHA4EJ8YxHn8DFaDisvhVlUOe4BF1tLD7GAj0lIqWVl+dPb/rExr0Btby5loQm20Q==");
    });
    ...
    it.each([
        ...
        ["moment", "git"],
        ["zod", "github"],
    ])("parses non-npm package %s", (pkgName, expectedType) => {
        ...
        strictEqual(pkg.type, expectedType);
        ...
        if (pkg.type === "tarball" && pkgName === "react") {
            ...
            deepStrictEqual(pkg.info, {
                dependencies: { "loose-envify": "^1.1.0" },
            });
        }
        if (pkg.type === "github") {
            if (pkgName === "dayjs") {
                strictEqual(pkg.user, "iamkun");
                strictEqual(pkg.repo, "dayjs#02b7a5c");
            }
            if (pkgName === "zod") {
                strictEqual(pkg.user, "colinhacks");
                strictEqual(pkg.repo, "zod#c3ec66c");
            }
        }
        if (pkg.type === "git") {
            // repo string should be present for git entries
            strictEqual(typeof pkg.repo === "string", true);
            strictEqual(pkg.repo.length > 0, true);
        }
    });
    ...
    it("parses duplicated-babel-frame fixture", () => {
        ...
        strictEqual(resolutions.some((r) => r.includes("@babel/code-frame@7.26.2")), true);
        strictEqual(resolutions.some((r) => r.includes("@babel/code-frame@7.27.1")), true);
        ...
        const topDeps = Object.keys(topBabelCodeFrame.info?.dependencies ?? {}).sort();
        deepStrictEqual(topDeps, ["@babel/helper-validator-identifier", "js-tokens", "picocolors"].sort());
        ...
        strictEqual(nestedBabelCodeFrame.resolution.includes("@babel/code-frame@7.27.1"), true);
        const nestedDeps = Object.keys(nestedBabelCodeFrame.info?.dependencies ?? {}).sort();
        deepStrictEqual(nestedDeps, ["@babel/helper-validator-identifier", "js-tokens", "picocolors"].sort());
        // no deep strict equal here. Ensure a samed resolved package installed in two places is the same object.
        strictEqual(packages.get("@babel/core/@babel/code-frame"), packages.get("@babel/template/@babel/code-frame"));
        // There should be two different package objects with name '@babel/code-frame'
        const codeFrameDependencies = [...packages.values()].filter((p) => p.type === "npm" && p.name === "@babel/code-frame");
        strictEqual(codeFrameDependencies.length, 4);
    });
});
//# sourceMappingURL=parseBunLockPackages.test.js.map