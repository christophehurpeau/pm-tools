 ...
 */
export const overrideKey = (packageName, convergence) => (convergence ? `${packageName}@` : packageName);
 ...
 */
export const addOverrides = (content, overrides, { convergence = true, comment } = {}) => {
...
export const readConvergenceOverrides = (content) => {
    const parsed = parseDocument(content).toJS();
    return new Map(Object.entries(parsed?.overrides ?? {}).flatMap(([key, value]) => key.endsWith("@") && typeof value === "string"
        ? [[key.slice(0, -1), value]]
        : []));
};
//# sourceMappingURL=pnpmWorkspaceYaml.js.map