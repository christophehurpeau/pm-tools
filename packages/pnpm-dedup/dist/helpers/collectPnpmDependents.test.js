...
import { collectPnpmDependents } from "./collectPnpmDependents.js";
...
describe("collectPnpmDependents", () => {
    ...
    it("collects both importer range and transitive resolved versions", () => {
        const dependents = collectPnpmDependents(lockFor("duplicated-babel-frame"), ["@babel/code-frame"]);
        const codeFrame = dependents.get("@babel/code-frame");
        ok(codeFrame);
        // direct importer dependency carries the specifier range
        ok(codeFrame.some((dependent) => dependent.key === "package.json in dependencies" &&
    ...
    it("restricts results to onlyPackageNames", () => {
        const dependents = collectPnpmDependents(lockFor("duplicated-printable-shell-command"), ["printable-shell-command"]);
        strictEqual([...dependents.keys()].every((name) => name === "printable-shell-command"), true);
        ok(dependents.get("printable-shell-command"));
    });
});
//# sourceMappingURL=collectPnpmDependents.test.js.map