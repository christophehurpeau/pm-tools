import { countDuplicatedPackages, createPackageFilter, diffVersionsSnapshots, renderDedupeSummary, } from "pm-utils";
...
import { readVersionsSnapshot } from "./helpers/versionsSnapshot.js";
...
const filteredResidualsMessage = "`pnpm dedupe` was not run: it ignores the package filter. Run `pnpm dedupe` yourself, or this command without a filter, to finish the rest.";
/**
 ...
 */
export function dedupe({ mode = "apply", convergenceOverrides = true, filter, } = {}) {
    ...
    const probe = dryRun && !filtered
        ? runPnpm(["dedupe", "--check"], { cwd: projectDir })
        : null;
    const residuals = (() => {
        if (filtered)
            return filteredResidualsMessage;
        if (probe !== null && probe.status !== 0)
            return residualsMessage;
        return undefined;
    })();
    ...
    if (filtered) {
        ...
        if (outcome.status === "applied" || outcome.status === "kept-overrides") {
            console.log("`pnpm dedupe` ran to apply the fixes above and merged what it could on its own: the filter bounds the edits made here, not pnpm's resolution.");
        }
