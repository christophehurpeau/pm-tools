...
const install = (cwd, ...args) => run(cwd, "pnpm", [
    "install",
    "--prefer-offline",
    "--ignore-scripts",
    ...args,
...
const dedupe = (cwd, ...args) => run(cwd, "node", [dedupeBin, ...args]);
...
suite("pnpm-dedupe end to end", () => {
    it("converges the cluster and leaves pnpm holding it without an override", () => {
        ...
        ok(applied.output.includes("Removing the overrides and re-resolving to check the result holds"), applied.output);
        ok(applied.output.includes("Cluster fixes: 4 duplicate resolutions -> 0"), applied.output);
        ...
        strictEqual(readFileSync(join(dir, "pnpm-workspace.yaml"), "utf8"), workspaceYamlContent);
