...
const lockFor = (scenario) => readPnpmLock(fileURLToPath(new URL(`../test/fixtures/${scenario}/pnpm-lock.yaml`, import.meta.url)));
const maxParenDepth = (id) => {
    let depth = 0;
    let max = 0;
...
const snapshotIds = (scenario) => Object.keys(lockFor(scenario).snapshots ?? {});
const duplicatesFor = (scenario) => filterDuplicatesPnpmPackagesMap(buildPnpmPackagesMap(parsePnpmLockPackages(lockFor(scenario))));
const fixesFor = (scenario) => {
    const lock = lockFor(scenario);
    const duplicates = duplicatesFor(scenario);
    return buildIdentifiedFixesMap(duplicates, collectPnpmDependents(lock, Object.keys(duplicates)));
};
describe("dedupePeers lockfiles", () => {
    ...
    it("flattens the peer suffixes the default settings nest", () => {
        ok(snapshotIds(defaultScenario).some((id) => maxParenDepth(id) > 1), "the default-settings fixture should contain nested peer suffixes");
        deepStrictEqual(snapshotIds(dedupePeersScenario).filter((id) => maxParenDepth(id) > 1), []);
    });
    it("parses name and version out of a flattened peer suffix", () => {
        const suffixed = snapshotIds(dedupePeersScenario).filter((id) => id.includes("("));
    ...
    it("groups every peer context of a resolution into installations", () => {
        const { packages, installationsByResolution } = parsePnpmLockPackages(lockFor(dedupePeersScenario));
        const utils = installationsByResolution.get("@typescript-eslint/utils@8.67.0");
        ok(utils);
        ok(utils.every((id) => id.startsWith("@typescript-eslint/utils@8.67.0")));
        strictEqual([...installationsByResolution.keys()].every((resolution) => packages.has(resolution)), true);
    });
    it("reports the same duplicated packages as the default-settings fixture", () => {
        deepStrictEqual(Object.keys(duplicatesFor(dedupePeersScenario)).toSorted(), Object.keys(duplicatesFor(defaultScenario)).toSorted());
    });
...
describe("dedupePeers with an identified fix", () => {
    it("flattens the peer suffixes of the pinned pair too", () => {
        ok(snapshotIds(mergeableDefaultScenario).some((id) => maxParenDepth(id) > 1));
        deepStrictEqual(snapshotIds(mergeableDedupePeersScenario).filter((id) => maxParenDepth(id) > 1), []);
    });
    ...
    it("identifies the same fixes as the default-settings fixture", () => {
        deepStrictEqual([...fixesFor(mergeableDedupePeersScenario)], [...fixesFor(mergeableDefaultScenario)]);
    });
});
//# sourceMappingURL=dedupePeers.test.js.map