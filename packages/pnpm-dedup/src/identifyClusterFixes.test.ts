...
const readStubbedManifest: ManifestReader = (name, version) => {
...
const clusterFixesFor = (
  scenario: string,
  readManifest: ManifestReader = readStubbedManifest,
): ClusterFix[] => {
...
const lockstepFixesFor = (
  scenario: string,
  readManifest: ManifestReader = readStubbedManifest,
): ClusterFix[] =>
  clusterFixesFor(scenario, readManifest).filter(
    (fix) => fix.members.length > 1,
  );

const metroCluster = (): ClusterFix => {
  const fix = clusterFixesFor("wildcard-not-reused").find((candidate) =>
    candidate.members.includes("metro"),
  );
...
describe("identifyClusterFixes", () => {
  ...
  const hoistedFixtureDir = fixturesBase(
    "../test/fixtures/hoisted-node-linker",
  );

  const hoistedClusterFix = (readManifest: ManifestReader): ClusterFix => {
    const lock = readPnpmLock(join(hoistedFixtureDir, "pnpm-lock.yaml"));
    const fixes = identifyClusterFixes(
      lock,
      buildPnpmPackagesMap(parsePnpmLockPackages(lock)),
      readManifest,
    );
  ...
  it("reads the ranges out of a hoisted node_modules", () => {
    ...
    deepStrictEqual(
      fix.reuseFixes.map(
        (reuse) =>
          `${reuse.requesterName}>${reuse.packageName} ${reuse.from} -> ${reuse.to}`,
      ),
      [
  ...
  const downgradeFixtureDir = fixturesBase(
    "../test/fixtures/exact-pin-forces-downgrade",
  );

  it("converges a lone duplicate on the version every range accepts", () => {
    const fixes = clusterFixesFor(
      "exact-pin-forces-downgrade",
      createManifestReader(downgradeFixtureDir),
    );
    const fix = fixes.find((candidate) =>
      candidate.members.includes("barcode-detector"),
    );
    ...
    deepStrictEqual(fix.workspaceChanges, []);
    deepStrictEqual(
      fix.externalConstraints.map((constraint) => [
  ...
  it("finds nothing for the duplicate a convergence drags along", () => {
    ...
    const fix = fixes.find((candidate) =>
      candidate.members.includes("zxing-wasm"),
    );
  ...
  it("keeps external constraints and drops the derived internal pins", () => {
    const byRequester = metroCluster().externalConstraints.map(
      (constraint) =>
    ...
    const requesterNames = new Set(
      metroCluster().externalConstraints.map(
        (constraint) => constraint.requesterName,
      ),
    );
  ...
  it("reports a cluster no member can leave as not dedupable", () => {
    ...
    // can collapse.
    const fix = clusterFixesFor("wildcard-not-reused").find((candidate) =>
      candidate.members.includes("hermes-parser"),
    );
    ok(fix);
    strictEqual(fix.applicable, false);
    strictEqual(fix.target, null);
