...
const readFixture = (scenario: string) =>
...
describe("parsePackageId", () => {
  ...
  it("strips the peer-dependency suffix", () => {
    deepStrictEqual(
      ...
      { name: "@typescript-eslint/utils", version: "8.61.0" },
    );
  });

  it("keeps a non-registry url as the version", () => {
    const parsed = parsePackageId(
      "zod@https://codeload.github.com/colinhacks/zod/tar.gz/ca42965df46b2f7e2747db29c40a26bcb32a51d5",
    );
...
describe("parsePnpmLockPackages", () => {
  ...
  it("parses both duplicated @babel/code-frame versions as npm", () => {
    const { packages } = parsePnpmLockPackages(
      readFixture("duplicated-babel-frame"),
    );
    const codeFrames = [...packages.values()].filter(
      (pkg) => pkg.name === "@babel/code-frame",
    );
  ...
  it("classifies non-registry resolutions as other, registry tarballs as npm", () => {
    ...
    const byName = new Map(
      [...packages.values()].map((pkg) => [pkg.name, pkg]),
    );
  ...
  it("groups peer-context snapshots into installations", () => {
    const { installationsByResolution } = parsePnpmLockPackages(
      readFixture("duplicated-typescript-eslint"),
    );
