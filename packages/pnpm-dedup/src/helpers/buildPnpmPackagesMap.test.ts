...
const buildFor = (scenario: string) =>
  buildPnpmPackagesMap(
...
const versionsOf = (
  packagesMap: ReturnType<typeof buildPnpmPackagesMap>,
  name: string,
): Set<string> =>
  new Set(
    (packagesMap[name] ?? []).map((resolution) => resolution.package.version),
  );
...
describe("buildPnpmPackagesMap", () => {
  it("reports no duplicates for the simple fixture", () => {
    ...
    strictEqual(
      Object.keys(filterDuplicatesPnpmPackagesMap(packagesMap)).length,
      0,
    );
  });
  ...
  it("detects the duplicated @babel/code-frame versions", () => {
    const duplicates = filterDuplicatesPnpmPackagesMap(
      buildFor("duplicated-babel-frame"),
    );
  ...
  it("detects the duplicated printable-shell-command versions", () => {
    const duplicates = filterDuplicatesPnpmPackagesMap(
      buildFor("duplicated-printable-shell-command"),
    );
  ...
  it("detects at least the known @typescript-eslint duplicates", () => {
    const duplicates = filterDuplicatesPnpmPackagesMap(
      buildFor("duplicated-typescript-eslint"),
    );
