...
import {
  addOverrides,
  overrideKey,
  readConvergenceOverrides,
} from "./pnpmWorkspaceYaml.ts";

const overrides = (entries: [string, string][]) => new Map(entries);
...
describe("pnpmWorkspaceYaml", () => {
  it("uses the empty range selector pnpm reads as a convergence override", () => {
    ...
    strictEqual(
      overrideKey("@react-native/metro-config", true),
  ...
  it("creates the file content when there is none", () => {
    strictEqual(
      addOverrides(undefined, overrides([["metro-config", "0.87.0"]])),
      'overrides:\n  "metro-config@": "0.87.0"\n',
    );
  });
  ...
  it("keeps comments, sibling keys and formatting", () => {
    ...
    strictEqual(
      addOverrides(content, overrides([["metro", "0.84.5"]])),
      [
        "# why this repo pins its resolution mode",
        "resolutionMode: time-based # inline",
        "",
  ...
  it("adds to an existing overrides block without touching its entries", () => {
    ...
    strictEqual(
      addOverrides(content, overrides([["bar", "2.0.0"]])),
      [
        "overrides:",
        "  # a decision taken elsewhere",
        '  "foo@": "1.0.0"',
  ...
  it("writes plain keys when convergence is disabled", () => {
    strictEqual(
      addOverrides(undefined, overrides([["metro", "0.84.5"]]), {
        convergence: false,
      }),
      'overrides:\n  "metro": "0.84.5"\n',
    );
  ...
  it("reads back only the convergence overrides", () => {
    const content = addOverrides(
      'overrides:\n  "plain-override": "1.0.0"\n',
      overrides([["metro", "0.84.5"]]),
    );

    deepStrictEqual(
      [...readConvergenceOverrides(content)],
      [["metro", "0.84.5"]],
    );
  });
});
