...
import { collectDependentRanges } from "./collectDependentRanges.ts";
import type { ManifestReader } from "./readInstalledManifest.ts";
import { createManifestReader } from "./readInstalledManifest.ts";
...
const readWildcardManifest: ManifestReader = (name, version) =>
  name === "@tamagui/metro-plugin" && version === "1.139.4"
...
const readDowngradeManifest = createManifestReader(
  fixturesBase(`../../test/fixtures/${downgradeScenario}`),
);

const rangeOf = (
  ranges: ReturnType<typeof collectDependentRanges>,
  packageName: string,
  dependentKey: string,
): string | undefined =>
  ranges.get(packageName)?.find((dependent) => dependent.key === dependentKey)
    ?.range;
...
describe("collectDependentRanges", () => {
  ...
  it("ignores packages that are not in the requested set", () => {
    deepStrictEqual(
      [...rangesFor("duplicated-babel-frame", ["@babel/code-frame"]).keys()],
      ["@babel/code-frame"],
    );
  });
  ...
  it("reads a peer range from the lockfile when no manifest is available", () => {
    ...
    strictEqual(
      rangeOf(ranges, "eslint", "@eslint-community/eslint-utils@4.9.1"),
      "^6.0.0 || ^7.0.0 || >=8.0.0",
    );
    strictEqual(
      rangeOf(ranges, "eslint", "@typescript-eslint/eslint-plugin@8.61.0"),
      "^8.57.0 || ^9.0.0 || ^10.0.0",
    );
  ...
  it("marks a dependent whose range came from a peer declaration", () => {
    const dependents =
      rangesFor("duplicated-typescript-eslint", ["eslint"]).get("eslint") ?? [];

    strictEqual(
      dependents.find(
        (dependent) => dependent.key === "eslint-plugin-n@17.24.0",
      )?.peer,
      true,
    );
    // a real dependency edge onto the same package stays unmarked
    strictEqual(
      rangesFor("duplicated-typescript-eslint", ["@eslint/plugin-kit"])
        .get("@eslint/plugin-kit")
        ?.find((dependent) => dependent.key === "eslint@10.5.0")?.peer,
      undefined,
    );
  });
  ...
  it("prefers a manifest declaration over the lockfile peer block", () => {
    strictEqual(
      rangeOf(
        rangesFor("duplicated-typescript-eslint", ["eslint"], (name, version) =>
          name === "@eslint-community/eslint-utils" && version === "4.9.1"
  ...
  it("strips the alias target from an importer's resolved version", () => {
    deepStrictEqual(
      rangesFor("mergeable-alias", ["printable-shell-command"])
  ...
  it("falls back to the resolved version when the manifest range is not comparable", () => {
    const readWorkspaceManifest: ManifestReader = (name, version) =>
      name === "@tamagui/metro-plugin" && version === "1.139.4"
        ? {
            dependencies: {
    ...
    strictEqual(
      rangeOf(ranges, "metro-config", wildcardDependent),
      // the resolved version, not `*` and not `workspace:*`
      "0.87.0",
    );
    strictEqual(
      rangeOf(ranges, "metro-transform-worker", wildcardDependent),
      "*",
    );
  });
});
