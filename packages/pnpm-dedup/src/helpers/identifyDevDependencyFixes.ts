...
const satisfiesAll = (version: string, ranges: string[]): boolean =>
  ranges.every((range) =>
 ...
 */
export const identifyDevDependencyFixes = (
  duplicatesPackagesMap: PackagesMap,
  dependentRanges: DependentRangesMap,
): DevDependencyFixesMap => {
  const fixes: DevDependencyFixesMap = new Map();
