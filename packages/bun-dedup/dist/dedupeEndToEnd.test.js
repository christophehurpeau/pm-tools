...
const fixtureDir = fileURLToPath(new URL("../test/fixtures/exact-pin-forces-downgrade", import.meta.url));
...
const install = (cwd, ...args) => run(cwd, ["install", ...args]);
const dedupe = (cwd, ...args) => run(cwd, [dedupeBin, ...args]);
const lockPath = (dir) => join(dir, "bun.lock");
const resolutionsOf = (dir, packageName) => {
    const packagesMap = buildPackagesMap(parseBunLockPackages(readAndParseBunLock(lockPath(dir))));
    return (packagesMap[packageName] ?? []).map(({ resolution }) => resolution);
};
const installedVersion = (dir, packageName) => JSON.parse(readFileSync(join(dir, "node_modules", packageName, "package.json"), "utf8")).version;
describe("bun-dedupe end to end", () => {
    it("merges onto the pinned version and leaves bun installing it", () => {
        ...
        ok(installedVersion(dir, "zxing-wasm").startsWith("2."), installedVersion(dir, "zxing-wasm"));
        // nothing was pinned into the manifest to hold it there
        strictEqual(readFileSync(join(dir, "package.json"), "utf8"), manifestBefore);
        ...
        ok(listed.some((line) => line.startsWith("zxing-wasm@2.")), list.output);
