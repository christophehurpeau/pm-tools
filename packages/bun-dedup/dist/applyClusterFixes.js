...
verifyFrozen = () => runBun(["install", "--frozen-lockfile"], { cwd: projectDir }).status, readFixes = defaultReadFixes, readDuplicates = readDuplicateSnapshot, filter, packageManagerResiduals, color = shouldColorize(), }) => {
    ...
    const frozenFailure = () => {
        if (verifyFrozen() === 0)
            return null;
        log("  `bun install --frozen-lockfile` rejects the result — reverting, the lockfile it produced is not one CI can reuse");
    ...
    if (returnedDuplicates.length > 0 || returnedReuses.length > 0) {
        log("  The overrides were the only thing holding the result: bun resolves back without them. A fix that needs a standing override is not one this tool applies.");
