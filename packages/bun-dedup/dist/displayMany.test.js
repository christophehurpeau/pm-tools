...
`);
    ...
    it("should display nested duplicates for typescript-eslint", () => {
        loadAndAssert("../test/fixtures/duplicated-typescript-eslint/bun.lock", `Found 16 duplicates:
...
`);
    ...
    it("renders the lockstep cluster fix for typescript-eslint", () => {
        const fixturePath = fileURLToPath(new URL("../test/fixtures/duplicated-typescript-eslint/bun.lock", import.meta.url));
        const bunLock = readAndParseBunLock(fixturePath);
        const packages = parseBunLockPackages(bunLock);
        const packagesMap = buildPackagesMap(packages);
        const duplicates = filterDuplicatesPackagesMap(packagesMap);
        const dependents = collectDependents(packages, bunLock.workspaces, Object.keys(duplicates));
        const clusterFixes = identifyClusterFixes(packagesMap, packages, bunLock.workspaces);
        let buffer = "";
        const log = (message = "") => {
            buffer += `${message}\n`;
        };
        ...
        expect(buffer).toContain('@pob/eslint-plugin requires @typescript-eslint/utils "8.59.1"');
    });
});
