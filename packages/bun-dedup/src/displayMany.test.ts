...
`,
  ...
  it("gives one line per package without --details", () => {
    expect(render("duplicated-babel-frame", { details: false })).toBe(
      `Found 1 duplicate:
...
`,
  ...
  it("names the downgrade a merge onto an installed copy takes", () => {
    ...
    expect(output).toContain(
      "barcode-detector — 2 versions, can be deduped to 3.0.3 (downgrade)\n",
    );
  ...
  it("lists every nested duplicate for typescript-eslint", () => {
    ...
    expect(output).toContain(
      '- eslint-plugin-import-x                      requires "^8.56.0"',
    );
  ...
  it("maps a resolution fix to the versions that would collapse", () => {
    ...
    expect(output).toContain(
      "printable-shell-command — 2 versions, can be deduped to 5.0.8 (upgrade)\n",
    );
    expect(output).toContain("  5.0.7  can be deduped to 5.0.8 (upgrade)\n");
    expect(lastLine(output)).toBe(
      "Found 6 dependencies, 1 duplicate, at least 1 dedupable (deduping may remove more). Run `bun-dedupe` to apply.",
    );
  });
  ...
  it("renders the lockstep cluster fix for typescript-eslint", () => {
    ...
    expect(output).toContain(
      "@typescript-eslint/types — 2 versions, can be deduped to 8.59.1 (downgrade) (cluster 1)\n",
    );
  });
  ...
  it("names the key an aliased dependent declares the package under", () => {
    ...
    expect(output).toContain(
      'package.json in devDependencies (as "@typescript/native")  requires "7.0.2"',
    );
