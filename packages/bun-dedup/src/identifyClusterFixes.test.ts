...
const loadFixtureClusterFixes = (relativeLockPath: string): ClusterFix[] => {
  const bunLock = readAndParseBunLock(
    fileURLToPath(new URL(relativeLockPath, import.meta.url)),
  );
...
describe("identifyClusterFixes", () => {
  const typescriptEslintFixes = () =>
    loadFixtureClusterFixes(
  ...
  it("keeps real external constraints and drops derived internal pins", () => {
    ...
    const byPackageAndRequester = fix.externalConstraints.map(
      (constraint) =>
    ...
    const requesterNames = new Set(
      fix.externalConstraints.map((constraint) => constraint.requesterName),
    );
  ...
  it("repoints an open range that ignored the pinned version", () => {
    const fixes = loadFixtureClusterFixes(
      "../test/fixtures/wildcard-not-reused/bun.lock",
    );
    expect(fixes).toHaveLength(1);
    const fix = fixes[0]!;
