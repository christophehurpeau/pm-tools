...
import type {
  NpmPackageResolution,
  PackageResolution,
} from "./helpers/buildPackagesMap.ts";
import type { Dependent, DependentsMap } from "./helpers/collectDependents.ts";
...
export function identifyResolutionFixes(
  ...
  const onlyNpmResolutions = resolutions
    ...
    .filter(
      (resolution) => resolution.package.type === "npm",
    ) as NpmPackageResolution[];
  ...
  for (const packageName of packageNames) {
    const npmVersions = onlyNpmResolutions
      ...
      .map((resolution) => ({
        ...
        satisfies: new Set<Dependent>(),
      }));

    const packageDependents = dependents.get(packageName);
    if (!packageDependents) {
      throw new Error(
        `Unexpected missing dependents for package ${packageName}`,
      );
    }
    ...
    const coveringCandidates = npmVersions.filter(
      (c) => c.satisfies.size === allDependentsCount,
    );
    ...
    if (coveringCandidates.length > 0) {
      ...
      const megeableResolutions = npmVersions
        ...
        .toSorted((a, b) => {
          const va = npmVersions.find(
            (n) => n.resolution.resolution === a,
          )?.version;
          const vb = npmVersions.find(
            (n) => n.resolution.resolution === b,
          )?.version;
    ...
    const unassigned = new Set<Dependent>(packageDependents);

    for (const candidate of npmVersions) {
      // dependents this candidate can satisfy that are still unassigned
      const satisfied = new Set<Dependent>(
        [...candidate.satisfies].filter((d) => unassigned.has(d)),
      );
      if (satisfied.size === 0) continue;
      ...
      for (const d of satisfied) unassigned.delete(d);
