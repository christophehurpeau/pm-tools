...
import type { Dependent } from "./collectDependents.ts";
import { collectDependents } from "./collectDependents.ts";
...
describe("collectDependents", () => {
  it("collects dependents for simple package", () => {
    ...
    expect(Object.fromEntries(dependents)).toEqual(
      loadDependentsFixture("semver-7.7.3.json"),
    );
  });
  ...
  it("finds dependents for @babel/code-frame in duplicated-babel-frame fixture", () => {
    ...
    expect(Object.fromEntries(dependents)).toEqual(
      loadDependentsFixture("babel-code-frame-7.26.2-7.27.1.json"),
    );
  });

  it("resolves the version a nested requester actually got", () => {
    const bunLock = readAndParseBunLock(
      fixturesBase("../../test/fixtures/duplicated-typescript-eslint/bun.lock"),
    );
    const packages = parseBunLockPackages(bunLock);
    const dependents = collectDependents(packages, bunLock.workspaces, [
      "semver",
    ...
    const byRequester = new Map(
      (dependents.get("semver") ?? []).map((dependent) => [
        dependent.key,
  ...
  it("keeps a caret range apart from the version it resolved to", () => {
    const bunLock = readAndParseBunLock(
      fixturesBase("../../test/fixtures/exact-pin-forces-downgrade/bun.lock"),
    );
    const packages = parseBunLockPackages(bunLock);
    const dependents = collectDependents(packages, bunLock.workspaces, [
      "barcode-detector",
    ...
    expect(
      (dependents.get("barcode-detector") ?? []).map((dependent) => [
        dependent.key,
        dependent.version,
        dependent.resolvedVersion,
      ]),
  ...
  it("resolves through aliased path segments", () => {
    const bunLock = readAndParseBunLock(
      fixturesBase("../../test/fixtures/aliased-nested/bun.lock"),
    );
    const packages = parseBunLockPackages(bunLock);
    const dependents = collectDependents(packages, bunLock.workspaces, [
      "semver",
    ...
    const resolvedBy = (packageName: string): Map<string, string | undefined> =>
      new Map(
        (dependents.get(packageName) ?? []).map((dependent) => [
          dependent.key,
          dependent.resolvedVersion,
        ]),
      );
  ...
  it("follows the declared key when an alias holds another package's name", () => {
    ...
    expect(
      (dependents.get("typescript") ?? []).map((dependent) => [
        dependent.key,
        dependent.version,
        dependent.resolvedVersion,
      ]),
    ...
    expect(
      (dependents.get("@typescript/typescript6") ?? []).map((dependent) => [
        dependent.key,
        dependent.version,
        dependent.resolvedVersion,
      ]),
  ...
  it("flags dependents whose declaration is not an npm range", () => {
    const bunLock = readAndParseBunLock(
      fixturesBase("../../test/fixtures/protocol-dependents/bun.lock"),
    );
    const packages = parseBunLockPackages(bunLock);
    const dependents = collectDependents(packages, bunLock.workspaces);
    ...
    expect(
      [...dependents].flatMap(([name, entries]) =>
        entries
          .filter((dependent) => dependent.nonSemver)
          .map((dependent) => [name, dependent.version]),
      ),
    ).toEqual([
      ["helper", "workspace:*"],
    ...
    expect(
      (dependents.get("helper") ?? []).map((dependent) => [
        dependent.key,
        dependent.version,
        dependent.resolvedVersion,
  ...
  it("keeps a version-less alias and flags git and tarball declarations", () => {
    const bunLock = readAndParseBunLock(
      fixturesBase("../../test/fixtures/non-npm/bun.lock"),
    );
    const packages = parseBunLockPackages(bunLock);
    const dependents = collectDependents(packages, bunLock.workspaces);
    ...
    for (const packageName of ["dayjs", "lodash", "moment", "react", "zod"]) {
      expect(
        (dependents.get(packageName) ?? []).map(({ nonSemver }) => nonSemver),
      ).toEqual([true]);
    }
  });
  ...
  it("finds dependents for printable-shell-command in duplicated-printable-shell-command fixture", () => {
    ...
    expect(Object.fromEntries(dependents)).toEqual(
      loadDependentsFixture("printable-shell-command-5.0.7-5.0.8.json"),
    );
  });
});
